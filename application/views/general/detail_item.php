		...
		<div class="tabbable tabs-below">
			<div id="myTabContent" class="tab-content">
				...
				<?php foreach ($gambar as $gbr) { ?>
					<div class="tab-pane fade" id="<?php echo $gbr['id_gambar'];?>">
						<div class="portfolio-item"><!------Thumnail Produk-------->
							<div class="item-inner" style="margin:0;">
								<img class="img-responsive" src="<?php echo base_url('doc/themes/public/img/produk/'.$gbr['gambar']); ?>" alt="">
								<div class="overlay">
									<a class="preview btn btn-danger" title="<?php echo $row['nama_produk']; ?>" href="<?php echo base_url('doc/themes/public/img/produk/'.$gbr['gambar']); ?>" rel="prettyPhoto"><i class="icon-eye-open"></i></a>
								</div>
							</div>
						</div><!---/thumbnail produk-->
					</div>
				...
				<ul id="myTab2" class="nav nav-tabs nav-justified">
					<li class="active col-xs-3" style="margin:0; padding:0;">
					...
					<?php $jml = count($gambar);
					if ($jml < 4){
						$sisa = 4-$jml;
					}
					?>
					<?php foreach ($gambar as $gbr) { ?>
						<li class="col-xs-3" style="margin:0; padding:0;">
							<a href="#<?php echo $gbr['id_gambar'];?>" data-toggle="tab">
								<img class="img-responsive" style="max-width:100%;" src="<?php echo base_url('doc/themes/public/img/produk/'.$gbr['gambar']); ?>" alt="">
							</a>
						</li>
						<?php }
						if (!empty($sisa)) {
...
<div id="comments">
	<div id="comments-list">
		<?php if (!empty($testimonial) OR !empty($mytestimonial)) { ?>
			<h3>Testimonial</h3>
			<?php
			...
			{
				//apakah sudah beli
				$SudahBeli = $this->m_produk->memberSudahBeli($Session['id_user'],$this->uri->segment(3));
				$TotalBeli = $SudahBeli->num_rows();
				if($TotalBeli>0)//sudah pernah beli
				{
					//count testimonial
					$countMyTestimoni = count($mytestimonial);
					if($countMyTestimoni < $TotalBeli){echo 'form tambah testimoni';}
					//echo my testimoni
					?>
					<?php foreach ($mytestimonial as $testi): ?>
						<div class="media">
							<div class="pull-left">
								<img class="avatar img-circle" style="max-width: 50px" src="<?php echo base_url('assets/img/photo/'.$testi['img_user']);?>" alt="">
							</div>
							<div class="media-body">
								<form method="POST" action="<?php echo site_url('proses_ubah/ubahTestimoni?id='.$testi['id_testi'])?>" >
									<div class="well">
										<div class="media-heading">
											<strong><?php echo $testi['nama']; ?></strong>&nbsp; <small><?php echo date('d F, Y - H:i', strtotime($testi['waktu'])); ?></small>
										</div>
										<textarea name="newTesti" class="form-control"><?php echo $testi['pesan']; ?></textarea><br/>
										<button style="float:left" type="submit" class="btn btn-default" ><i class="icon-repeat"></i> Ubah Testimoni</button>
									</div>
								</form>
							</div>
						</div><!--/.media-->
						<hr/>
					<?php endforeach;//end of my testimoni ?>
					<?php
				}else{
					echo 'testimoni hanya bisa diberikan setelah membeli barang';
				}
			}
			//get testimoni except user login
			?>
...
<script type="text/javascript">
    ...
    function stokCheck(){
        url = '<?php echo site_url("proses/prosesJatuhTempo"); ?>';
        $.ajax({
            url:url,
